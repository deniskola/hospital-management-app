...
const Sidebar = props => {
  ...
  const menuItemsJSX = menuItems.map((item, index) => {
      ...
      return(
        <Link to={`${item.to}${subMenuItem.to}`} style={{textDecoration:'none'}} key={subMenuItemIndex}>
          <s.subMenuItem
            ...
            selected={isSubmenuItemSelected} colorPalette={colorPalette}
            >
              {subMenuItem.name}
            </s.subMenuItem>
        </Link>
    ...
    return (
      <s.ItemContainer key={index}>
        ...
        <AnimatePresence>
          {hasSubmenus && isOpen && (
            <motion.nav initial={{opacity:0,y:-15}} animate={{opacity:1,y:0}} transition={{duration:0.35}} exit={{opacity:0,x:-30}}>
              <s.SubMenuItemContainer isSidebarOpen={isSidebarOpen} colorPalette={colorPalette}>
                {subMenusJSX}
  ...
  return (
    <s.SidebarContainer isSidebarOpen={isSidebarOpen} colorPalette={colorPalette}>
